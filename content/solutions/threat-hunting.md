...
A hunt is a bounded investigation against one or more connected SIEMs. It can start from a threat brief, a modeled risk, an ethical hack finding, or an analyst prompt. CraftedSignal compiles the query for the target platforms, tracks each SIEM result separately and records retries or failures without losing the overall hunt state.
...
Hunts are the bridge between "we might be exposed" and "we have proven coverage." They are useful before writing a rule, after an ethical hack, when a new threat brief lands, or when a rule has gone silent and the team needs to know whether the behavior still exists.
